...
class SnakeGame(object):
    ...
    def render(self):
        # Render each pixel using 3 by 2 blocks to account for aspect ratio.
        ...
        output += BORDER_HOR_CHAR * (pixel_width*width+2) + "\n"
        for j in range(height):
            row = ""
            row += BORDER_VERT_CHAR
            for i in range(width):
                if (i, j) in self.game_state.filled_squares:
                    char = self.game_state.filled_squares[(i, j)]*pixel_width
                    row += char
                else:
                    row += " "*pixel_width

            row += BORDER_VERT_CHAR + "\n"
            output += row*pixel_height
            #print(row)
        output += BORDER_HOR_CHAR * (pixel_width*width+2) + "\n"
        #print(BORDER_HOR_CHAR * (width+2))
        print(output)
...
class NStepLookaheadPlayer(Player):
    ...
    def next_move(self, game_state):
        ...
        if self.last_move:
            moves = set(moves)
            oppos = OPPOSITE_MOVES[self.last_move]
            if oppos in moves:
                moves.remove(oppos)
...
width = 64
height = 32
snake_game = SnakeGame(NStepLookaheadPlayer(lookahead_steps=100), width, height, max_frame_rate=75)
